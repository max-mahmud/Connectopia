...
const CommentForm = ({ user, id, replyAt, setCommentId }) => {
  ...
  const onSubmit = async (data) => {
    if (replyAt) {
      dispatch(reply_comment({ comment: data.comment, id, replyAt: replyAt, from: user.firstName }));
    } else {
      dispatch(comment_post({ comment: data.comment, id, from: user.firstName }));
    }
    setCommentId(id);
  };
  return (
...
const PostCard = ({ post, user, loader, userData }) => {
  ...
  useEffect(() => {
    if (commentId !== null) {
      dispatch(get_comments(commentId));
    }
  }, [commentId, dispatch]);
        ...
        <div>
          <p className="text-ascent-2">
            ...
            {post?.description?.length > 301 &&
              (showAll === post?._id ? (
                <span className="text-blue ml-2 font-mediu cursor-pointer" onClick={() => setShowAll(0)}>
                  Show Less
                </span>
              ) : (
                <span
      ...
      text-base border-t border-[#66666645]"
        ...
        {showComments === post?._id && (
          <div className="w-full mt-4 border-t border-[#66666645] pt-4 ">
            ...
            {comments?.length > 0 && comments[0]?.postId === post?._id ? (
              comments?.map((comment) => (
                <div className="w-full py-2" key={comment?._id}>
                  ...
                  <div className="ml-12">
                    ...
                    <div className="mt-2 flex gap-6">
                      <p className="flex gap-2 items-center text-base text-ascent-2 cursor-pointer"></p>
                      <span
                    ...
                    {replyComments === comment?._id && (
                      <CommentForm user={user} id={comment?._id} loader={loader} replyAt={comment?.from} />
                    )}
                  </div>
